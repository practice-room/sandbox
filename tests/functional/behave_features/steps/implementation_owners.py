...
def impl_chart_name(context, chart_name: str):
    unique_name = context.owners_file_test.set_chart_name(chart_name)
    logging.info(f'unique name for chart was "{unique_name}"')
...
def impl_label_and_name(context, vendor_label: str, vendor_name: str):
    logging.debug(f'vendor label was: {vendor_label}')
    logging.debug(f'vendor name was: "{vendor_name}"')
    ...
    context.owners_file_test.create_and_commit_redhat_owners_file()
...
def impl_ci_conclusion(context, result: str):
    # context.owners_file_test.check_workflow_conclusion('success')
